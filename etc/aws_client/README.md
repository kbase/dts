...
The example runs using AWS and a Minio instance, which must be running locally. To start the Minio instance run:
...
In a separate bash terminal, from this folder run:
```bash
go run ./...
```
